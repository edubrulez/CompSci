...
BEGIN {compute # of gallons needed and total cost of paint}
      tempgal:= RA / CPG; {gal. needed = room area / coverage per gallon}
   {cannot buy decimal gallons so must move up to next integer}
      IF tempgal > trunc(tempgal)
         THEN GN:= trunc(tempgal) + 1
         ELSE GN:= trunc(tempgal);
